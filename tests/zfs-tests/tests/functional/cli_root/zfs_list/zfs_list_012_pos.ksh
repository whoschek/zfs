...
{
	log_must eval "zfs list -H -p -t snapshot -o name,userrefs,quota " \
...
function verify_default_property_lists
{
	...
	for object_types in snapshot snapshot,bookmark; do
		log_must eval "zfs list -H -p -t '$object_types' " \
		    "-o name,used,available,referenced,mountpoint,quota " \
		    "'$DATASET' | cut -f1-5 > '$EXPECTED_OUTPUT'"
		log_must eval "LD_PRELOAD='$preload' " \
		    "ZFS_SNAPSHOT_LIST_TEST_MODE='count' " \
		    "ZFS_SNAPSHOT_LIST_TEST_MARKER='$MARKER' " \
		    "zfs list -H -p -t '$object_types' '$DATASET' " \
		    "> '$INJECTED_OUTPUT'"
...
function verify_colored_snapshot_listing_uses_legacy
{
	...
	for object_types in snapshot snapshot,bookmark; do
		...
		log_must eval "faketty TERM=xterm-256color ZFS_COLOR=1 " \
		    "zfs list -t '$object_types' -s quota '$DATASET' " \
		    "> '$EXPECTED_OUTPUT'"
		log_must eval "faketty TERM=xterm-256color ZFS_COLOR=1 " \
		    "LD_PRELOAD='$preload' " \
		    "ZFS_SNAPSHOT_LIST_TEST_MODE='count' " \
		    "ZFS_SNAPSHOT_LIST_TEST_MARKER='$MARKER' " \
		    "zfs list -t '$object_types' '$DATASET' " \
		    "> '$INJECTED_OUTPUT'"
		[[ ! -e "$MARKER" ]] || log_fail \
...
function verify_projected_redacted
{
	...
	log_must eval "zfs list -H -p -t snapshot " \
	    "-o '$columns,quota' '$REDACT_RECV' | cut -f1-5 " \
	    "> '$EXPECTED_OUTPUT'"
	log_must eval "LD_PRELOAD='$preload' " \
	    "ZFS_SNAPSHOT_LIST_TEST_MODE='count' " \
	    "ZFS_SNAPSHOT_LIST_TEST_MARKER='$MARKER' " \
...
function verify_mixed_type_fallback
{
	...
	log_must eval "zfs list -H -p -t snapshot,bookmark -d 1 " \
	    "-o '$columns,quota' '$DATASET' | cut -f1-3 " \
	    "> '$EXPECTED_OUTPUT'"
