...
check_snapshot_stats(dsl_pool_t *dp, uint64_t dsobj, const char *description,
    uint64_t injected_object, uint64_t min_txg, int expected_error,
    boolean_t expected_valid, boolean_t expected_injection)
{
...
main(int argc, char **argv)
{
	...
	*separator = '\0';
	...
	if (check_snapshot_stats(dp, dsobj, "normal written lookup", 0, 0,
	    0, B_TRUE, B_FALSE) != 0 ||
	    check_snapshot_stats(dp, dsobj, "current deadlist EIO",
	    deadlist_obj, 0, EIO, B_FALSE, B_TRUE) != 0 ||
	    check_snapshot_stats(dp, dsobj, "predecessor EIO", prev_obj, 0, 0,
	    B_FALSE, B_TRUE) != 0 ||
	    check_snapshot_stats(dp, dsobj, "filtered current deadlist EIO",
	    deadlist_obj, creation_txg + 1, 0, B_FALSE, B_FALSE) != 0) {
		error = EPROTO;
	}
