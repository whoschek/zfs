...
/*
 * Calculate written bytes for a snapshot.  Current deadlist errors are
 * returned, while predecessor errors leave written invalid.  The configuration
 * lock keeps the deadlist object and predecessor linkage stable.
 */
static int
    ...
    uint64_t min_txg, uint64_t max_txg, dsl_dataset_snapshot_stats_t *stats)
{
	...
	if (error == 0 && want_written && dsp->ds_prev_snap_obj != 0 &&
		...
		error = dsl_dataset_snapshot_written(dp, dsp,
