...
/*
 * Calculate written bytes for a snapshot without constructing dsl_dataset_t
 * state.  The configuration lock keeps predecessor linkage and the snapshot's
 * immutable deadlist stable while their bonus buffers are read.
 */
static int
dsl_dataset_snapshot_written(dsl_pool_t *dp,
    const dsl_dataset_phys_t *dsp, uint64_t *written)
{
	objset_t *mos = dp->dp_meta_objset;
	dmu_buf_t *dbuf;
	dmu_object_info_t doi;
    ...
    uint64_t min_txg, uint64_t max_txg, dsl_dataset_snapshot_stats_t *stats)
{
	...
	if (error == 0 && want_written && dsp->ds_prev_snap_obj != 0 &&
	    (min_txg == 0 || dsp->ds_creation_txg >= min_txg) &&
	    (max_txg == 0 || dsp->ds_creation_txg <= max_txg) &&
	    dsl_dataset_snapshot_written(dp, dsp, &stats->dss_written) == 0) {
		stats->dss_written_valid = B_TRUE;
	}
